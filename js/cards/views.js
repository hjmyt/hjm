...
function renderDetailContent(c, own) {
    ...
    const feed = `<section class="profile-panel" id="cardFeeding"><h3 class="panel-title">${I('gift')}投喂 · ${c.giftTitle || (c.id === 'qiqi' ? '大姐姐的下午茶' : (c.id === 'orange' ? '给它' : '给他') + '一份小心意')}<span class="tiny">今日还可投喂 ${3 - used}/3</span></h3>${CardUI.response ? `<div class="feed-response" role="status">${escapeHTML(CardUI.response)}</div>` : ''}<div class="gift-grid">${gifts.map(g => `<button class="gift-choice ${CardUI.gift === g[0] ? 'selected' : ''}" data-card-gift="${g[0]}" aria-pressed="${CardUI.gift === g[0]}" ${used >= 3 ? 'disabled' : ''}>${I(g[2])}<span>${g[1]}</span><small>${g[3]} ♪ · 羁绊分 +${g[4]}</small></button>`).join('')}</div><div class="feeding-bottom"><div class="feeding-progress"><div class="progress-label"><span>羁绊分</span><span>${bond} / 100</span></div><div class="progress-track"><div class="progress-fill" style="width:${Math.min(100, bond)}%"></div></div></div><button class="btn primary" id="cardFeedBtn" data-card-feed ${!selected || used >= 3 || state.coins < (selected?.[3] || 0) ? 'disabled' : ''}>${I('heart')}投喂${selected ? ` ${selected[3]} ♪` : ''}</button></div><p class="feed-hint" id="cardGiftHint">${used >= 3 ? '今天已经收到了三份心意，明天再见。' : selected ? `这份心意：羁绊分 +${selected[4]}、经验 +${selected[5]}${state.coins < selected[3] ? '。音符不足，读故事或演奏可以获得。' : '。'}` : '选择一份礼物，再点击投喂。音符可以通过故事与演奏获得。'}</p><div class="growth-strip"><div class="growth-copy">Lv.${lv} · ${lv === 60 ? '已满级' : `经验 ${v.xp % 60} / 60`}<small>合奏获得经验；每 60 经验提升 1 级，最高 60 级。</small></div><button class="btn secondary small" data-card-train="${c.id}" ${lv === 60 || state.coins < 8 ? 'disabled' : ''}>${I('stats')}练习 +30 经验 · 8 ♪</button></div></section>`;
