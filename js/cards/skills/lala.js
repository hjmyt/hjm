...
function lalaFeeding(c) {
    ...
    return `<section class="profile-panel" id="cardFeeding"><h3 class="panel-title">${I('gift')}投喂 · 排练后的休息站<span class="tiny">今日剩余 ${Math.max(0, 3 - used)}/3</span></h3>${CardUI.response ? `<div class="feed-response" role="status">${escapeHTML(CardUI.response)}</div>` : ''}<div class="gift-grid">${gifts.map(a => `<button class="gift-choice ${a[0] === CardUI.gift ? 'selected' : ''}" data-card-gift="${a[0]}" aria-pressed="${a[0] === CardUI.gift}" ${used >= 3 ? 'disabled' : ''}>${I(a[2])}<span>${a[1]}</span><small>${a[3]} ♪ · 羁绊分 +${a[4]}</small></button>`).join('')}</div><div class="feeding-bottom"><div class="feeding-progress">${trioProgress('羁绊分', bond)}</div><button class="btn primary" id="cardFeedBtn" data-card-feed ${!g || used >= 3 || state.coins < (g?.[3] || 0) ? 'disabled' : ''}>${I('heart')}投喂${g ? ' ' + g[3] + ' ♪' : ''}</button></div><p class="feed-hint" id="cardGiftHint">${used >= 3 ? '今天的心意已收好，明天再一起排练。' : g ? '羁绊分 +' + g[4] + ' · 经验 +' + g[5] + '。' : '选一份心意，让照顾大家的她也好好休息一下。'}</p><p class="stat-caption">她拉好二提，也随时准备为三提补位；哪里需要一个声部，她就稳稳接上。</p><div class="growth-strip"><div class="growth-copy">Lv.${lv} / 60 · 羁绊分 ${bond}/100<small>每 60 经验升 1 级；礼物内容为游戏设定。</small></div><button class="btn ghost small" data-card-train="lala" ${state.coins < 8 || lv >= 60 ? 'disabled' : ''}>${I('violin')}短练习 · 8 ♪</button></div></section>`;
}
