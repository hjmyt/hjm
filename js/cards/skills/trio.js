...
function trioFeeding(c) {
    ...
    return `<section class="profile-panel" id="cardFeeding"><h3 class="panel-title">${I('gift')}投喂 · ${c.feedingStyle}<span class="tiny">今日剩余 ${Math.max(0, 3 - used)}/3</span></h3>${CardUI.response ? `<div class="feed-response" role="status">${escapeHTML(CardUI.response)}</div>` : ''}<div class="gift-grid">${gifts.map(a => `<button class="gift-choice ${a[0] === CardUI.gift ? 'selected' : ''}" data-card-gift="${a[0]}" aria-pressed="${a[0] === CardUI.gift}" ${used >= 3 ? 'disabled' : ''}>${I(a[2])}<span>${a[1]}</span><small>${a[3]} ♪ · 羁绊分 +${a[4]}</small></button>`).join('')}</div><div class="feeding-bottom"><div class="feeding-progress">${trioProgress('羁绊分', bond)}</div><button class="btn primary" id="cardFeedBtn" data-card-feed ${!g || used >= 3 || state.coins < (g?.[3] || 0) ? 'disabled' : ''}>${I('heart')}投喂${g ? ' ' + g[3] + ' ♪' : ''}</button></div><p class="feed-hint" id="cardGiftHint">${g ? '羁绊分 +' + g[4] + ' · 经验 +' + g[5] + '。' : '选择一份心意，再点击投喂。'}</p><p class="trio-tiny-rule">${hints}<br>礼物价格与养成奖励为本版游戏数值，不涉及真实购买。</p><div class="trio-state cool"><b>合奏精力</b><div class="trio-energy-row">${[...new Set([c.id, ...state.cards.team])].map(id => `<span>${cardDef(id).name}<strong>${trioEnergy(id)}/100</strong></span>`).join('')}</div><small>新增的本地状态：每场有命中的演奏消耗 10 点；投喂个人 +10。关怀可补充、首席的低节奏反噬会减少。降到 0 仍可继续玩，不扣琴技或原成绩。</small></div><div class="growth-strip"><div class="growth-copy">Lv.${cardLevel(c.id)} / 60 · 羁绊分 ${bond}/100<small>每 60 经验升级；每日可投喂三次。</small></div><button class="btn ghost small" data-card-train="${c.id}" ${state.coins < 8 || cardLevel(c.id) >= 60 ? 'disabled' : ''}>短练习 · 8 ♪</button></div></section>`;
}
