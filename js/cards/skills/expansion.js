...
function expansionFeeding(c) {
    ...
    return `<section class="profile-panel" id="cardFeeding"><h3 class="panel-title">${I('gift')}投喂 · ${c.feedingStyle}<span class="tiny">今日 ${3 - used}/3 次</span></h3>${isDouble ? '<div class="double-gift">emo 恢复中 · 本次投喂可恢复心情，收到后解除 Debuff</div>' : ''}${CardUI.response ? `<div class="feed-response" role="status">${escapeHTML(CardUI.response)}</div>` : ''}<div class="gift-grid">${gifts.map(g => `<button class="gift-choice ${CardUI.gift === g[0] ? 'selected' : ''}" data-card-gift="${g[0]}" aria-pressed="${CardUI.gift === g[0]}" ${used >= 3 ? 'disabled' : ''}>${I(g[2])}<span>${g[1]}</span><small>${g[3]} ♪ · 羁绊分 +${g[4]}${isDouble ? '' : ''}</small></button>`).join('')}</div><div class="feeding-bottom"><div class="feeding-progress"><div class="progress-label"><span>羁绊分</span><span>${bond} / 100</span></div><div class="progress-track"><div class="progress-fill" style="width:${Math.min(100, bond)}%"></div></div></div><button class="btn primary" id="cardFeedBtn" data-card-feed ${!selected || used >= 3 || state.coins < (selected?.[3] || 0) ? 'disabled' : ''}>${I('heart')}投喂${selected ? ' ' + selected[3] + ' ♪' : ''}</button></div><p class="feed-hint" id="cardGiftHint">${selected ? `这份心意：羁绊分 +${selected[4]}、经验 +${selected[5]}。` : '选择一份心意，再点击投喂。'} 正传与卡册共用。</p><div class="growth-strip"><div class="growth-copy">Lv.${lv} / 60 · ${lv >= 60 ? '已满级' : `距离升级 ${60 - v.xp % 60} 经验`}<small>短练习：8 音符 → 30 经验；等级影响演出奖励，不改写原始属性。</small></div><button class="btn ghost small" data-card-train="${c.id}" ${lv >= 60 || state.coins < 8 ? 'disabled' : ''}>${I('music')}短练习 8 ♪</button></div></section>`;
}
