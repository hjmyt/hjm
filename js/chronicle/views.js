...
function createChronicleViews(ctx) {
    ...
    function dialogueHTML(d, label) {
        return d.parts.map((p, i) => `<section class="cp-dialogue-turn">${speaker(p.who, i ? '接着说' : label)}<div class="cp-text">${ctx.E(p.text)}</div></section>`).join('');
    }
    function sceneFrameHTML(content) {
        ...
        return `<article class="cp-novel ${art ? 'cp-novel-illustrated' : 'cp-novel-text-only'}" aria-label="正传故事" ${art ? `style="--scene-art:url('${new URL(ASSETS[art.asset], document.baseURI).href}')"` : ''}>
          ...
          ${art ? `<div class="cp-novel-visual"><div class="cp-novel-heading"><span>这一刻，留在故事里</span><h3>${ctx.E(title)}</h3><button class="cp-memory-link" data-memory="${art.id}">${I('album')}已收入回忆 · 查看</button></div><figure class="cp-story-art cp-novel-art"><button data-memory="${art.id}" aria-label="查看回忆：${ctx.E(title)}"><img src="${ASSETS[art.asset]}" alt="${ctx.E(art.text)}" decoding="async"></button><figcaption>${ctx.E(location)}</figcaption></figure></div>` : ''}
    ...
    function choicesHTML(d) { return `<div class="cp-choices">${d.choices.map((c, i) => `<button class="cp-choice" data-cp-choice="${i}" data-cp-rev="${ctx.R().rev}" ${c.disabled ? 'disabled' : ''}><span class="cp-option-n">${String(i + 1).padStart(2, '0')}</span><span>${ctx.E(c.text)}<small>${ctx.E(ctx.choiceBondHint(i))}</small></span>${I('arrow')}</button>`).join('')}</div>`; }
    ...
    function weekAdvanceHTML() {
        ...
        return `<section class="cp-week-advance" aria-label="演出周进度"><div class="cp-week-progress-copy"><span class="cp-week-kicker">${left ? '本章日程 · 每周一段故事' : '演出周已到达'}</span><strong>${ctx.stageName()}</strong><span class="cp-week-count">${left ? `还有 <b>${left}</b> 周` : '现在可以继续剧情'}</span></div><ol class="cp-week-track" aria-label="当前第 ${r.week} 周">${Array.from({ length: 6 }, (_, i) => `<li class="${r.week === i + 1 ? 'current' : r.week > i + 1 ? 'done' : ''}" ${r.week === i + 1 ? 'aria-current="step"' : ''}><span></span>第 ${i + 1} 周<small>${ctx.E(ctx.weekPlan(r, i + 1).title)}</small></li>`).join('')}</ol><div class="cp-week-end"><div class="cp-week-message" role="status" aria-live="polite" aria-atomic="true"><strong>${advanced ? `${I('check')}已从第 ${ctx.weekNotice.from} 周进入第 ${r.week} 周` : `当前是第 ${r.week} 周`}</strong><p>${left ? (r.weekly.done.includes(r.week) ? '本周故事已记录。可以继续自由活动，也可以进入下一周。' : '先完成本周主线，再进入下一周；可选相遇不必全部完成。') : '演出剧情已开启，准备好后即可继续。'}</p></div>${left ? actionButton('end-week', r.week === 5 ? '进入第 6 周剧情' : '前往第 ' + (r.week + 1) + ' 周', 'arrow', 'primary', r.weekly.done.includes(r.week) ? '' : 'disabled') : actionButton('to-live', '继续演出剧情', 'music', 'primary')}</div></section>`;
    }
    function weeklyHTML() {
        const r = ctx.R();
        return `<div class="cp-surface"><div class="cp-overline" style="color:#a38b64">WEEK ${String(r.week).padStart(2, '0')} · YOUR REHEARSAL DAYS</div>${ctx.weekStoryHTML()}${ctx.sideStoriesHTML()}<h4 class="cp-free-time-title">余下的时间，留给你</h4><div class="cp-actions-grid">${[
            ['practice', 'violin', '刻苦练琴', '10 音符 · 琴技 +2', state.coins < 10], ['ensemble', 'team', '自由合练', '选择搭档 · 免费考核 · 不推进周数', false], ['social', 'heart', '找人聊天', '每日陪伴 +1 · 也听听其他声部', false], ['earn', 'music', '演奏赚音符', '免费演奏 · 达到 C 可获得音符', false]
        ].map(([action, icon, title, sub, disabled]) => `<button class="cp-action-card" data-cp-action="${action}" data-cp-rev="${r.rev}" ${disabled ? 'disabled' : ''}>${I(icon)}<strong>${title}</strong><small>${sub}</small></button>`).join('')}</div>${weekAdvanceHTML()}<div class="cp-rule-note">${I('help')}完成本周主线后可前往下一周。练琴、聊天和可选相遇不会自动推进时间；第 6 周进入演出剧情，琴技达到 ${ctx.requiredTech()} 后可以上台。</div></div>`;
    }
    ...
    function practiceHTML() {
        ...
        if (r.scene === 'practice_partner' || !b?.partner) {
            return `<div class="cp-surface">${sceneIntroHTML(`${speaker('kongge', '练习对决 · 选择搭档')}<h3>首席空格的考核</h3><div class="cp-text">曲目《欢乐颂》难度 12。\n选择一位搭档，与你一起把这一段合上。</div>`)}<div class="cp-partners">${['lala', 'azhe', 'shiyuan', 'tim', 'yeshiyang', 'feihong', 'dijie'].map(k => partnerHTML(k, 'partner')).join('')}</div><div class="cp-rule-note">${I('team')}正传原有搭档保留，并新增 TIM、叶思阳、垃垃；不需要先抽到对应卡牌。这里使用<strong>全局羁绊分</strong>，与卡册同步。羁绊达到 3 后，搭档合奏得分更高。</div></div>`;
        }
        if (r.scene === 'practice_result') {
            return `<div class="cp-surface">${speaker('kongge', '考核结果已存档')}<div class="cp-battle-result">${b.win ? 'PASSED' : 'TRY AGAIN'}</div><div class="cp-text">${b.win ? '（合上谱子）「合格。下周还来。」' : '「……回家把这几小节练二十遍。」\n考核失败，但路还长。'}</div><div class="cp-reward">${b.win ? '考核通过：乐团等级 +1；每日首次通过 +3 音符，不增加琴技。' : '这次没有考核奖励，不会清空你已有的进度。'}<br>当前琴技 ${r.tech} · 音符 ${state.coins} · 乐团 Lv.${r.level}</div>${actionButton('practice-continue', '继续剧情', 'arrow', 'primary')}<p class="cp-caption">结果已结算；刷新或返回页面不会重复领取奖励。</p></div>`;
        }
