...
function createChronicleViews(ctx) {
    const preloadedSceneArt = new Map();
    function preloadUpcomingArt() {
        const nextScenes = new Set((ctx.dialogue()?.choices || []).map(choice => choice.next).filter(next => typeof next === 'string'));
        for (const art of CHRONICLE_ART) {
            if (art.chapter !== ctx.R().chapter || !nextScenes.has(art.scene)) continue;
            const src = ASSETS[art.asset];
            if (!src || preloadedSceneArt.has(src)) continue;
            const image = new Image();
            image.decoding = 'async';
            image.src = src;
            preloadedSceneArt.set(src, image);
        }
    }
    ...
    function sceneFrameHTML(content) {
        ...
        return `<article class="cp-novel ${art ? 'cp-novel-illustrated' : 'cp-novel-text-only'}" aria-label="正传故事" ${art ? `style="--scene-art:url('${new URL(ASSETS[art.asset], document.baseURI).href}')"` : ''}>
          ...
          ${art ? `<div class="cp-novel-visual"><div class="cp-novel-heading"><h3>${ctx.E(title)}</h3><button class="cp-memory-link" data-memory="${art.id}">${I('album')}已收入回忆 · 查看</button></div><figure class="cp-story-art cp-novel-art"><button data-memory="${art.id}" aria-label="查看回忆：${ctx.E(title)}"><img src="${ASSETS[art.asset]}" alt="${ctx.E(art.text)}" width="440" height="440" decoding="async" fetchpriority="high"></button><figcaption>${ctx.E(location)}</figcaption></figure></div>` : ''}
    ...
    function practiceHTML() {
        ...
        if (r.scene === 'practice_partner' || !b?.partner) {
            const duo=r.flags.feiSide&&(cardOwned('baoshi')||r.flags.c4bao)?`<button class="cp-partner" data-cp-action="partner" data-cp-person="baoshi_feihong" data-cp-rev="${r.rev}"><span class="cp-partner-duo"><img src="${ASSETS.cardBaoshi}" alt="宝石"><img src="${ASSETS.cardFeihong}" alt="飞鸿"></span><span><strong>宝石＋飞鸿</strong><small>主唱与和声 · 胜利可提升乐团等级</small></span></button>`:'';
            return `<div class="cp-surface">${sceneIntroHTML(`${speaker('kongge', '练习对决 · 选择搭档')}<h3>首席空格的${r.battle?.context === 'weekly' ? '阶段验收' : '入团摸底'}</h3><div class="cp-text">曲目《欢乐颂》难度 12。\n选择搭档，与你一起把这一段合上。</div>`)}<div class="cp-partners">${['lala', 'azhe', 'shiyuan', 'tim', 'yeshiyang', 'feihong', 'dijie'].map(k => partnerHTML(k, 'partner')).join('')}${duo}</div><div class="cp-rule-note">${I('team')}正传原有搭档保留，并新增 TIM、叶思阳、垃垃；不需要先抽到对应卡牌。替飞鸿解围且结识宝石后，可选两人一起合练；验收通过会提升乐团等级，并记录宝石飞鸿乐队线前置。</div></div>`;
        }
