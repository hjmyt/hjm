...
function createChronicleChaptersFiveSix(ctx) {
    ...
    function lateChapterDialogue(G, meta) {
        ...
        switch (G.scene) {
            ...
            case 'c6_zhu': return ctx.D('zhu', '首演筹备开始后。朱老师几乎住在了琴房，咖啡杯摞成塔，稿子改到第七版。有天深夜你路过，看见他对着空谱纸发呆，小声嘀咕：「为什么这里……就是写不下去呢……」', ctx.choice('进去陪他改谱', 'c6_warn', () => ctx.affUp('zhu', 3)), ctx.choice('帮他校对和声', 'c6_warn', () => { ctx.affUp('zhu', 3); ctx.plotTech(1); }), ctx.choice('不打扰，回去睡觉', 'c6_warn'));
    ...
    function lateTitleHTML() { const r = ctx.R(), e = ctx.ENDINGS[r.ending]; return `<div class="cp-surface"><div class="cp-ending-hero">${I(e?.icon || 'album')}<div class="cp-overline">${e?.code || 'CHAPTER COMPLETE'}</div><h3>${ctx.E(e?.title || ctx.chapterName() + ' · 完')}</h3><p class="cp-caption">${ctx.E(e?.text || '这一章的旅程已经保存。')}</p>${r.live ? `<p class="cp-caption">演出得分 ${r.live.score} / 难度 ${r.live.diff}</p>` : ''}</div><div class="cp-choices">${e ? `<button class="cp-choice gold" data-memory="${e.memory}"><span>重看这次落幕<small>人物、回忆与结局奖励已保存</small></span>${I('album')}</button>` : ''}${r.chapter === 5 ? '<button class="btn primary" data-cp-action="switch-chapter" data-cp-chapter="6">继续第六章 · 开幕之夜 →</button>' : '<div class="cp-teaser"><strong>第七章 · 个人线</strong>演出落幕之后，去听一个人的故事。<br>阿喆与十元个人线已开放；对应角色羁绊需超过 35 分。<br><button class="btn primary" data-cp-action="personal-picker">选择角色，进入第七章 →</button></div>'}${ctx.actionButton('restart', '从本章起点重选', 'repeat', 'secondary')}<button class="btn secondary" data-cp-action="switch-chapter" data-cp-chapter="${r.chapter - 1}">返回前一章</button><button class="btn secondary" data-route="cards">看看相遇的伙伴</button></div></div>`; }
