...
function createChronicleChaptersTwo(ctx) {
    ...
    function groupDialogue(G, introduced = false) {
        const intro = introduced ? '那晚，音乐人 Jerry 自己来到了山丘酒吧。他只看了一场排练就给出诊断：「一锅烩，必死。要分组。」\n\n黑板被分成两半——左边写着「弦乐组」，右边写着「流行组」。朱老师沉默良久，点了点头。史称：哈基米之夜。\n\n' : '';
        const record = () => ctx.reward('c2_night');
        const chooseStrings = () => { record();G.flags.strGroup = 1;G.flags.popGroup = 0;G.flags.bandGroup = 0;G.flags.yangcun = 0;ctx.log('选择弦乐组；如在十元线选择只组乐团，可分流至宝石×飞鸿。'); };
        const chooseBand = () => { record();G.flags.strGroup = 0;G.flags.popGroup = 0;if(G.level>=6){G.flags.bandGroup = 1;G.flags.yangcun = 1;ctx.log('进入乐队组「羊村」，剧情位置已保留。');}else{G.flags.bandGroup = 0;G.flags.yangcun = 0;G.flags.popGroup = 1;ctx.ending('c2_street');} };
        const bandText=G.level>=6?'乐队组 · 羊村（宝石＋飞鸿）':`乐队组 · 当前 Lv.${G.level}，未达 Lv.6 将进入「街头卖唱」`;
        const text=G.level>=6?'乐团已经到达 Lv.6，有能力支撑乐队组。现在选择你要站到的一边：':`全团的目光落在你身上。弦乐组可以正常进入；乐队组需要 Lv.6，当前为 Lv.${G.level}。你仍然可以自己决定要站到哪一边。`;
        return ctx.D(introduced ? 'jerry' : 'narrator', intro + text, ctx.choice('弦乐组 · 继续十元与独立路演方向', 'c2_str', chooseStrings), ctx.choice(bandText, G.level>=6?'c2_yangcun':'c2_pop_end', chooseBand));
    }
    function chapterTwoDialogue(run = ctx.R()) {
        ...
        switch (G.scene) {
            ...
            case 'c2_night_pre': return groupDialogue(G, true);
            case 'c2_night': return groupDialogue(G, false);
            case 'c2_yangcun': return ctx.D('narrator', '你、宝石和飞鸿把谱架搬到了同一边。新乐队的名字暂定为「羊村」。\n\n（羊村乐队专属剧情待补充。本节点已独立记录，后续更新不需重选分组。）', ctx.choice('确认乐队组 · 继续现有筹备', 'c2_head'), ctx.choice('返回黑板 · 重新选择分组', 'c2_night', () => { G.flags.bandGroup = 0;G.flags.yangcun = 0;G.flags.strGroup = 0;G.flags.popGroup = 0; }));
            case 'c2_str': return ctx.D('narrator', '你站到了弦乐组一边。垃垃看了你一眼，没有说话，但你感觉有什么东西被悄悄放进了心里。\n\n后来的路还很长。音乐剧、路演、越来越大的舞台——都从这个小小的角落，开始生长。', ctx.choice('确认弦乐组 · 跟上垃垃', 'c2_head'), ctx.choice('返回黑板 · 重新选择分组', 'c2_night', () => { G.flags.bandGroup = 0;G.flags.yangcun = 0;G.flags.strGroup = 0;G.flags.popGroup = 0; }));
    ...
    function chapterTwoHelp() { return `<p>${ECONOMY_HELP}</p><div class="cp-rule-table"><strong>章节衔接</strong><span>第一章结束后解锁第二章，沿用入团登记并继承琴技、等级、羁绊与黑化值；第二章周数从 1 开始。六章依次解锁、分别保存，点击顶部已解锁章节卡即可来回切换。</span><strong>哈基米之夜</strong><span>玩家始终可以自行选择弦乐组或乐队组。弦乐组不受乐团等级限制；选择乐队组时才检查等级：低于 6 级进入「街头卖唱」，达到 6 级进入宝石与飞鸿所在的「羊村」。羊村专属剧情已保留独立节点，待后续补写。</span><strong>周常与三条支线</strong><span>练琴消耗 10 音符，琴技 +2；音符不足可先免费演奏。合作提议在开场连贯呈现，Bill 缺席在第 5 周出现；TIM 工作时间消息在第 2 周起作为可选相遇开放。每个赞助节点的音符跨重开只领取一次；事件每周目可重新选择，之后可查看记录；已处理事件保留记录，不重复结算。</span><strong>路演</strong><span>第 6 周开放，需琴技 14。难度 12 + 登台周数，共 5 段，最高 50 分。判定与第一章相同：PERFECT +10、GOOD +6、区外 +2。延期加练实际推进一周；没有强制倒计时。</span><strong>存档与重选</strong><span>保存对白节点、决策、事件、合练回合、路演每段及光标位置。离开页面／打开弹窗会暂停，不会自动判定。重开第二章只恢复本章起点，第一章不动；结局奖励与回忆跨周目保留。</span></div><p class="cp-caption">结局与人物线索会随你的选择自然揭晓。</p>`; }
