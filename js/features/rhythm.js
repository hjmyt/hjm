...
function renderGameOverlay() {
    ...
    if (game.status === 'idle') {
        ...
        const r = game.result, run = game.cardRun;
        ...
        e.innerHTML = `<div class="eyebrow">OUR LITTLE ENCORE</div><h3 class="result-rank">${r.rank}</h3><h3 style="font-size:18px;margin-top:8px">${r.accuracy >= 85 ? '这一次，我们很有默契。' : r.accuracy >= 45 ? '你认真演奏的样子，很好看。' : '每一首合奏，都从第一拍开始。'}</h3><div class="result-stats"><div><strong>${game.score}</strong><span>本次得分</span></div><div><strong>${r.accuracy}%</strong><span>准确率</span></div><div><strong>${game.maxCombo}</strong><span>最高连击</span></div></div><p style="margin:0 0 17px">${r.reward ? `收获 ${r.reward} 音符 · ${r.newBest ? '刷新个人最佳！' : '谢谢你的认真演奏。'}` : '完整演奏达到 C（45%）即可获得音符，试试简单模式再来一次。'}</p>${teamResult}${run?.trioReport ? '<p class="result-extra">' + escapeHTML(run.trioReport) + '</p>' : ''}${run?.sourceReport ? `<p class="result-extra">${escapeHTML(run.sourceReport)}</p>` : ''}${run?.lalaReport ? `<p class="result-extra">${escapeHTML(run.lalaReport)}</p>` : ''}${run?.ids.includes('azhe') && run?.credited ? '<button class="btn secondary small" data-azhe-applause style="margin-bottom:8px">为阿喆送上掌声 · 华彩安可</button>' : ''}<div class="result-buttons"><button class="btn primary small" data-game="start">${I('repeat')}再演一遍</button><button class="btn secondary small" data-route="home">回排练室</button></div>`;
    }
