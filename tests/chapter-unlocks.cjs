...
(async () => {
 ...
 try {
  ...
  const checks=await page.evaluate(async()=>{
   ...
   await choice(0);await choice(0);await choice(4);await choice(0);check('First week ends before the audition',state.chronicle.run.scene==='menu'&&!cardOwned('kongge'));await click('[data-cp-action="end-week"]');check('Kongge unlocks on his first dialogue',state.chronicle.run.scene==='s_first'&&cardOwned('kongge'));
   ...
   state.cards.pity=9;const guaranteed=drawOneCard();check('SSR pity still works',['SSR','UR'].includes(CARD_DEFS.find(c=>c.id===guaranteed.id).rarity));
   ...
   state.affinity.tim=35;goCard('tim');check('Ready skill stays collapsed',!!$('view-card').querySelector('details:not([open])')&&!$('view-card').innerText.includes('女朋友'));
   ...
   expansion().azhe.tickets=1;state.affinity.azhe=35;renderCardPage();check('Azhe unlocked skill stays collapsed',$('view-card').innerHTML.includes('华彩安可')&&!$('view-card').innerText.includes('华彩安可'));
   await enroll();await choice(0);await choice(0);await choice(0);await choice(4);await choice(0);await click('[data-cp-action="end-week"]');check('Skipping look scene keeps optional people locked',exact(['lala','shiyuan','zhu','kongge']));
