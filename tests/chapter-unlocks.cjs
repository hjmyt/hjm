...
(async () => {
 ...
 try {
  ...
  const checks=await page.evaluate(async()=>{
   ...
   reset();route('chronicle');await click('[data-cp-action="switch-chapter"][data-cp-chapter="2"]');$('cpQuickName').value='测试';await click('[data-cp-action="start-second"][data-cp-mode="quick"]');await choice(4);await choice(0);
   check('Chapter two quick start unlocks narrator but not seeded affinity',state.chronicle.run.scene==='c2_intro'&&exact(['lala','zhu']));
