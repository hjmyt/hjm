...
(async()=>{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  scene(2,'title3',{strGroup:1});Object.assign(state.chronicle.run,{ending:'c2_retry',ch:3,tech:17,gold:93,level:4});state.chronicle.run.aff.tim=12;state.chronicle.endings=['c2_retry'];save();route('chronicle');await click('[data-cp-action="switch-chapter"][data-cp-chapter="3"]');check('Second chapter failure can continue with inherited growth',state.chronicle.run.chapter===3&&state.chronicle.run.tech===17&&state.chronicle.run.gold===93&&state.chronicle.run.aff.tim===12&&state.chronicle.run.week===1&&state.chronicle.slots['2'].ending==='c2_retry');
