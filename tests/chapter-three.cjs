...
(async()=>{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  reset();route('chronicle');check('Six main chapters plus chapter-seven entrance',document.querySelectorAll('.cp-chapter-tile').length===7);
  ...
  scene(2,'title3',{strGroup:1});Object.assign(state.chronicle.run,{ending:'c2_retry',ch:3,tech:17,gold:93,level:4});state.chronicle.run.aff.tim=12;state.chronicle.endings=['c2_retry'];save();route('chronicle');await click('[data-cp-action="switch-chapter"][data-cp-chapter="3"]');check('Second chapter failure can continue with inherited growth',state.chronicle.run.chapter===3&&state.chronicle.run.tech===17&&!('gold' in state.chronicle.run)&&state.chronicle.run.aff.tim===12&&state.chronicle.run.week===1&&state.chronicle.slots['2'].ending==='c2_retry');
  ...
  scene(3,'c3_ge');state.cards.encounters.push('qiqi');save();state.cards.team=['qiqi','shiyuan'];state.affinity.qiqi=35;goCard('qiqi');check('Qiqi hidden bond is absent before condition',!$('view-card').innerHTML.includes('姐妹同框')&&!$('view-card').innerHTML.includes('好姐妹'));
