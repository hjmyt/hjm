...
(async()=>{const browser=await chromium.launch({headless:true,...(process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE?{executablePath:process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE}:{})});try{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  setup();Object.assign(state.chronicle.run,{chapter:3,ch:3,scene:'s_dream',aff:{...state.chronicle.run.aff,shiyuan:98}});state.chronicle.bonds.shiyuan=98;state.chronicle.run.rev++;renderGlobal();await click('[data-cp-choice="2"]');check('Shared BE stores originating chapter',state.chronicle.chapterEndings[3].includes('shadow'));
