...
- `node tests/weekly-stories.cjs`：真实点击六章每周主线、未完成推进防护、提前结局、可选相遇、旧存档迁移幂等、重开、三张附件加载与桌面/手机布局。章节分支测试使用已完成周计划的独立节点夹具；完整周流程由此用例覆盖。

- `node tests/album-art.cjs`：113 段回忆图片映射、64 张原相册独立插图、未解锁保护、旧收藏刷新、桌面/手机完整构图、详情与纪念图下载。

- `node tests/chronicle-art.cjs`：43 张不同场景插图、46 个场景绑定、到达收藏、分支隔离、旧档补齐与幂等、刷新保存、相册链接、四种屏宽的 24 个布局组合。
