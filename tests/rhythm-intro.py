...
raw=subprocess.check_output(['ffmpeg','-v','error','-i',str(root/'assets/audio/rhythm/love-hakimi-op-full.mp3'),'-ac','1','-ar','22050','-f','f32le','-'])
...
assert abs(len(samples)/22050-157.8535)<.05
...
print('PASS: 3-second silent countdown, immediately audible 2.5-second music prelude, complete original duration.')

if len(sys.argv)>1:
    raw_source=subprocess.check_output(['ffmpeg','-v','error','-i',sys.argv[1],'-ac','1','-ar','22050','-f','f32le','-'])
    original=array('f');original.frombytes(raw_source)
    for start,end in [(.5,5.5),(12,17),(24,29),(60,65),(100,105),(149,154.8)]:
        ...
        assert cov/den>.98, 'Full recording must match source 00:00 without drift in each section'
