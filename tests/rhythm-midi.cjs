...
(async()=>{
 ...
 assert.equal(report.sourceAudioSha256,crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'assets/audio/rhythm/love-hakimi-op-preview.mp3'))).digest('hex'));
 assert(report.segments.length>=6&&report.segments.every(s=>s.samples>=4));
 assert.equal(report.chartBasis,'audio-percussion');
 assert(report.bpm>142&&report.bpm<145);
 for(const a of [0,7,14,21])assert(report.events.some(e=>e.time>=a&&e.time<a+7&&e.main),'Audible main drum beats in each quarter');
 for(const n of report.events) assert(Math.abs(n.time-(n.gridTime+n.localOffsetSeconds))<.061&&Math.abs(n.time-n.audioOnset)<.0006&&n.strength>=.85,'Every target uses an observed percussion attack, including the latter half');
 ...
 try{
  ...
  const checks=await page.evaluate(()=>{
   ...
   route('rhythm');check('Default OP with preserved legacy catalog slots',game.track===2&&TRACKS[0].name==='晚风排练曲'&&TRACKS[1].name==='猫步小夜曲');
   const track=TRACKS[2];check('Drum chart version',track.chartVersion==='opening-drums-v1'&&track.audioSourceStart===0);
   for(const mode of ['gentle','normal']){
    const chart=track.charts[mode];check('Chart bounds '+mode,chart.every(([t,l],i)=>t>=.25&&t<27.3&&Number.isInteger(l)&&l>=0&&l<4&&(!i||t>=chart[i-1][0])));
   ...
   const finish=(raw,{team=false,complete=true,manual=true}={})=>{
    ...
    Object.assign(game,{status:'running',duration:28,elapsed:complete?28:2,notes:Array.from({length:100},()=>({})),score:raw*1000,perfect:100,good:0,nice:0,miss:0,combo:100,maxCombo:100,cardRun:run});
   ...
   state.best['love-hakimi-op-preview-v1_normal']={score:124000,accuracy:100,rank:'S',combo:124};state.best['0_gentle']={score:1234,accuracy:60,rank:'C',combo:2};state.best['1_normal']={score:2345,accuracy:70,rank:'B',combo:3};save();return results;
  });
  await page.reload();assert(await page.evaluate(()=>game.track===2&&state.best['0_gentle'].score===1234&&state.best['1_normal'].score===2345&&state.best['love-hakimi-op-opening-drums-v1_normal'].score>0&&state.best['love-hakimi-op-preview-v1_normal'].score===124000));
  for(const width of [1440,390]){await page.setViewportSize({width,height:900});await page.evaluate(()=>route('rhythm'));await page.locator('[data-mode="normal"]').click();assert.match(await page.locator('#rhythmTeam').textContent(),/7\/8\/9\/10/);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'/tmp/hjm-midi-rewards-'+width+'.png',fullPage:true});}
