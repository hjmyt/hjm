...
(async()=>{
 ...
 try {
  ...
  const checks=await page.evaluate(async()=>{
   ...
   await choose(0);await click('[data-cp-action="end-week"]');check('Camera dialogue assigned to Tang',$('cpStoryText').querySelector('[data-speaker="tangshao"]')&&!$('cpStoryText').textContent.includes('荷鲁斯之眼'));
   ...
   await choose(0);await click('[data-cp-action="end-week"]');await choose(1);check('Score proofreading adds skill and Zhu bond',state.chronicle.run.tech===21&&state.chronicle.run.aff.zhu===19);
   ...
   const before=state.coins;await click('[data-cp-action="restart"]');await click('[data-cp-action="confirm-restart"]');check('Restart retains bonds and no ordering',state.chronicle.run.aff.zhu===19&&state.chronicle.run.scene==='c6_intro'&&!document.querySelector('.shanqiu-menu'));await finish([10,10,10,10,10]);check('No repeated ending reward',state.coins===before);
