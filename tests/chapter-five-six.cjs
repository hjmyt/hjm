...
(async()=>{
 ...
 try {
  ...
  const checks=await page.evaluate(async()=>{
   ...
   setup(1,'start');check('Six sequential chapter tabs',document.querySelectorAll('.cp-chapter-tile').length===6&&document.querySelector('[data-cp-chapter="5"]').disabled&&document.querySelector('[data-cp-chapter="6"]').disabled);
   ...
   await choose(0);await choose(0);check('Fifth weekly loop',state.chronicle.run.scene==='menu');
   await click('[data-cp-action="social"]');await click('[data-cp-person="huangyx"]');check('Huang chat adds global bond',state.chronicle.run.aff.huangyx===2&&$('cpStoryText').textContent.includes('合作方案'));await choose(0);
   state.chronicle.run.aff.shiyuan=98;state.chronicle.bonds.shiyuan=98;await click('[data-cp-action="social"]');await click('[data-cp-person="shiyuan"]');check('Late romance not interrupted at 99',!state.chronicle.run.ending&&state.chronicle.run.aff.shiyuan===100);await choose(0);
   ...
   await click('[data-cp-action="switch-chapter"][data-cp-chapter="6"]');check('Sixth carries global bonds without drinks',state.chronicle.run.scene==='c6_intro'&&state.chronicle.run.aff.zhu===14&&state.chronicle.run.aff.huangyx===2&&!document.querySelector('.shanqiu-menu'));
   await choose(0);await choose(1);check('Score proofreading adds skill and Zhu bond',state.chronicle.run.tech===21&&state.chronicle.run.aff.zhu===17);
   check('Five unique prior HE recognized',$('cpStoryText').textContent.includes('5 场演出 HE'));
   await choose(0);await choose(0);await finish([10,10,10,10,10]);check('Musical HE recorded',state.chronicle.run.ending==='c6_he');await choose(0);check('Season finale does not expose chapter seven',document.querySelector('#cpMain').textContent.includes('正传第一季 · 完')&&!document.querySelector('[data-cp-chapter="7"]'));
   ...
   const before=state.coins;await click('[data-cp-action="restart"]');await click('[data-cp-action="confirm-restart"]');check('Restart retains bonds and no ordering',state.chronicle.run.aff.zhu===17&&state.chronicle.run.scene==='c6_intro'&&!document.querySelector('.shanqiu-menu'));await finish([10,10,10,10,10]);check('No repeated ending reward',state.coins===before);
