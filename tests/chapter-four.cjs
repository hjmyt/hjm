...
(async()=>{
 ...
 try{
  ...
  const checks=await page.evaluate(async()=>{
   ...
   scene('menu',{}, {tim:70});await click('[data-cp-action="end-week"]');serialize();await click('[data-cp-action="end-week"]');check('Gossip counters persist without retired mechanics',state.chronicle.run.flags.qBack===2&&state.chronicle.run.aff.tim===70&&state.chronicle.run.tech===15&&!('discontent' in state.chronicle.run));
   ...
   await click('[data-cp-action="switch-chapter"][data-cp-chapter="4"]');check('Carry preserves growth and plot flags',state.chronicle.run.tech===22&&state.chronicle.run.gold===99&&state.chronicle.run.flags.qBack===1&&state.chronicle.run.flags.taIn===1&&state.chronicle.slots[3].ending==='c3_he');
