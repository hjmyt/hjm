...
(async()=>{
 ...
 try{
  ...
  const nextStory=async()=>{await click('[data-cp-action="end-week"]');};
   ...
   scene('c4_band_lemon',{}, {lemon:10});await choose(2);check('Explicit trip triggers BE',state.chronicle.run.ending==='c4_lemon'&&state.chronicle.run.scene==='be_mianbei');serialize();await choose(0);check('Lemon ending can advance to title',state.chronicle.run.scene==='title5');await click('[data-cp-action="restart"]');await click('[data-cp-action="confirm-restart"]');check('Lemon ending restart preserves bonds',state.chronicle.run.aff.lemon===10&&state.chronicle.run.scene==='c4_intro');
   ...
   scene('menu',{}, {tim:70});await click('[data-cp-action="end-week"]');serialize();await click('[data-cp-action="end-week"]');check('High bonds do not create gossip penalties',!state.chronicle.run.flags.qBack&&state.chronicle.run.aff.tim===70&&state.chronicle.run.tech===15&&!('discontent' in state.chronicle.run));
   ...
   scene('s_dream',{}, {shiyuan:98});await choose(2);check('Explicit shadow ending stays fourth',state.chronicle.run.ending==='shadow'&&state.chronicle.run.scene==='be_shiyuan');await choose(0);check('Shadow links fourth title',state.chronicle.run.scene==='title5');
   ...
   await click('[data-cp-action="switch-chapter"][data-cp-chapter="4"]');check('Carry preserves growth and plot flags',state.chronicle.run.tech===22&&!('gold' in state.chronicle.run)&&state.chronicle.run.flags.qBack===1&&state.chronicle.run.flags.taIn===1&&state.chronicle.slots[3].ending==='c3_he');
