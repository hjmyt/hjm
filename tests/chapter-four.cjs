...
(async()=>{
 ...
 try{
  ...
  const checks=await page.evaluate(async()=>{
   ...
   await click('[data-cp-action="switch-chapter"][data-cp-chapter="4"]');$('cpQuickName').value='剧场新人';await click('[data-cp-action="start-fourth"][data-cp-mode="quick"]');
   ...
   check('Intro unlocks only Yeshiyang',state.cards.encounters.join()==='yeshiyang');
   ...
   await click('[data-cp-action="restart"]');await click('[data-cp-action="confirm-restart"]');check('Restart restores entrance and retains cards',state.chronicle.run.scene==='c4_intro'&&state.chronicle.run.tech===14&&!state.chronicle.run.flags.yangCompromise&&cardOwned('baoshi'));
   ...
   state.chronicle.run.flags.qBack=2;state.chronicle.run.flags.yangSolo=1;await click('[data-cp-action="restart"]');await click('[data-cp-action="confirm-restart"]');check('Restart restores inherited flags to entrance snapshot',state.chronicle.run.flags.qBack===1&&!state.chronicle.run.flags.yangSolo&&state.chronicle.run.tech===22);
