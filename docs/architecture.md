...
默认进入页面即尝试有声自动播放并循环；浏览器禁止自动播放时，首次点击或按键后启动。保留已保存的关闭配乐、总静音和音量偏好，页面后台暂停。普通页面的控制栏与剧情控制栏共用开关、音量及原有 `hjm-story-bgm-v1` 偏好；总声音关闭时暂停。保留 file:// 播放，单播放器避免重叠。音频按用户提供的原文件直接复制，无裁剪、无额外转码，时长约 154.85 秒，使用媒体 `loop` 循环；SHA-256：`5574c5589309260dde6dba5be6d697c10e41ecb2534ed0a1b4f0edab8af150c0`。
